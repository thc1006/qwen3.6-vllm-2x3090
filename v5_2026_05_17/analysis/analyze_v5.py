...
# ---- zh-TW / zh-CN detection (bidirectional OpenCC; aggregate.json shipped
# in this directory was regenerated by this script after the v5.0 ERRATA fix
# — the previously-shipped numbers used a unidirectional classifier that
# collapsed SIMP into "shared". The corrected counts are in the README. ----
try:
    ...
    def trad_status(text):
        """Bidirectional: has TRAD-only chars iff t2s changes it; has SIMP-only
        chars iff s2t changes it. Both → MIX; neither → shared (Han chars all
        shared between TW/CN). Empty / no-Han → none."""
        if not text:
            return "none"
        ...
        if not han:
            return "none"
        s = "".join(han)
        has_trad = _cc_t2s.convert(s) != s
        has_simp = _cc_s2t.convert(s) != s
        if has_trad and has_simp:
            return "MIX"
        if has_trad:
            return "TRAD"
        if has_simp:
    ...
    _TRAD = set(
        "為麼樂書買賣應記車沒動來裡東問發現說話長樣這點國還學時間見點開關電腦進這對體當頭裡會學業講願氣個歲時錢愛網過內導語給東說語經當還動覺車員樣國連風機鳳權門總獨見覺識記實業課專塊試廠處員議週節電興奮樂業務識歲時點問題見錯誤節謝認識為"
    )
    _SIMP = set(
        "为么乐书买卖应记车没动来里东问发现说话长样这点国还学时间见点开关电脑进这对体当头里会学业讲愿气个岁时钱爱网过内导语给东说语经当还动觉车员样国连风机凤权门总独见觉识记实业课专块试厂处员议周节电兴奋乐业务识岁时点问题见错误节谢认识为"
    )
